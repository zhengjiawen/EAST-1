...
def eval_model(model_name, test_img_path, submit_path, save_flag=True):
    ...
    res = subprocess.getoutput('zip -q submit_vgg.zip *.txt')
    res = subprocess.getoutput('mv submit_vgg.zip ../')
    os.chdir('../')
    res = subprocess.getoutput('python ./evaluate/script.py –g=./evaluate/gt.zip –s=./submit_vgg.zip')

    print(res)
    os.remove('./submit_vgg.zip')
...
if __name__ == '__main__':
    ...
    submit_path = './submit_vgg'
    # eval_model(model_name, test_img_path, submit_path)
    model_format = './pths_vgg16/model_epoch_{}.pth'
    precision = []
    recall = []
    hmean = []
    save_name = './pths_vgg16/model_test_{}.jpg'
    save_path = './pths_vgg16/{}.npy'
    for i in range(20, 901, 20):
    ...
    drawTestAcc('VGG16 ', precision, recall, hmean, save_name.format('VGG16'))
